...
function normaliseMessages(messages, imageLimit) {
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    for (const attachment of Array.isArray(message.attachments) ? message.attachments : []) {
      ...
      const image = imageFromAttachment(message, attachment);
      const key = attachment.id || image.url;
...
async function getDiscordGallery(env = {}) {
  ...
  try {
    const messages = await fetchDiscordMessages(token, channelId, 100);
...
module.exports = { getDiscordGallery };
